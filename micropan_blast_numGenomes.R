...
#changed from 180 to just 50 because of exponencial complexity issues
sq <- seq(10,60,10)
...
fin <- mclapply(sq, function(d){
   ...
   df <- mclapply(1:5, function(i){
    set.seed(i)
    gfs <- sample(gffs, d)
    out <- paste0('micropan_blast_resu/',d,'Genomes_out_',i,'/')
    dir.create(out)
...
saveRDS(fin, file = 'micropan_blast_resu/resu_numGenomes.RDS')
