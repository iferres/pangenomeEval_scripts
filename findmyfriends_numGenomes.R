...
fin <- mclapply(sq, function(d){
  ...
  df <- mclapply(1:10, function(i){
    set.seed(i)
    gfs <- sample(gffs, d)
    out <- paste0('findmyfriends_resu/',d,'Genomes_out_',i,'/')
    dir.create(out)
    ...
    sapply(gfs, function(x){
      ...
      ff <- gsub('#','_',(sub('gff$', 'faa', basename(x))))
      ff <- grep(paste0(ff, '$'), faas, value = TRUE)
      rl <- readLines(ff)
      rl[grep('^>', rl)] <- nhe
...
saveRDS(fin, file='findmyfriends_resu/findmyfriends_numGenomes.RDS')
